...
class StockDataFetcher:
    ...
    def _generate_multi_timeframe_charts(self, ticker: str, valid_ticker: str, w52_high=None, w52_low=None) -> list:
        ...
        chart_periods = [
            ('1w', '1mo'),  # 1 month (more data than 7d)
            ('3m', '3mo'),  # 3 month  
            ('1y', '1y'),   # 1 year
        ]
        ...
        print(f"   [CHART] Starting chart generation for {ticker} (valid: {valid_ticker})")
        
        for period_name, yf_period in chart_periods:
            try:
                print(f"   [CHART] Fetching {yf_period} data for {valid_ticker}")
                
                # Fetch data for this period - suppress stderr
                old_stderr = sys.stderr
                sys.stderr = io.StringIO()
                try:
                ...
                print(f"   [CHART] Got {len(hist)} rows for {period_name}")
                
                if hist.empty or len(hist) < 5:
                    print(f"   [!] Not enough data for {period_name}: {len(hist)} rows")
                    continue
                ...
                if chart_path:
                    ...
                    print(f"   [CHART] Generated {period_name} chart for {ticker}: {chart_path}")
                else:
                    print(f"   [!] _generate_single_chart returned None for {period_name}")
        ...
        print(f"   [CHART] Finished chart generation for {ticker}: {len(chart_paths)} charts")
        return chart_paths
    ...
    def _generate_single_chart(self, ticker: str, hist_data, w52_high=None, w52_low=None, period_name='3m') -> Optional[str]:
        ...
        Returns the file path to the saved PNG or None on failure.
        """
        print(f"   [CHART] _generate_single_chart called for {ticker} period={period_name}, data rows={len(hist_data) if hist_data is not None else 'None'}")
        ...
        try:
            ...
            print(f"   [CHART] Columns in data: {list(hist_copy.columns)}")
            ...
            # Custom style
            mc = mpf.make_marketcolors(up='#26a69a', down='#ef5350', inherit=True)
            period_labels = {'1w': '1 Month', '3m': '3 Month', '1y': '1 Year'}
            style = mpf.make_mpf_style(
                marketcolors=mc,
                gridstyle='-',
                gridcolor='#e0e0e0',
            ...
            kwargs = dict(
                ...
                style=style,
                volume=True,
                title=f'${ticker} - {period_labels.get(period_name, period_name)} Chart',
                savefig=dict(fname=filepath, dpi=120, bbox_inches='tight'),
                figsize=(10, 8),
            )
            if addplots:
    ...
    def get_stock_data(self, ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Generate multiple timeframe charts if enabled
            if ENABLE_CHARTS:
                try:
